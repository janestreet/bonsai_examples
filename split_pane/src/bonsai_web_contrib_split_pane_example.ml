...
open! Vdom
module Parameters = Bonsai_web_contrib_split_pane.For_testing.Parameters
...
let create_demo ~parameters (local_ graph) =
  let first_panel =
  ...
  in
  let pane =
    Bonsai_web_contrib_split_pane.For_testing.create_from_parameters
      parameters
      ~first_panel
      ~second_panel
      graph
  ...
  let%arr pane in
  let view =
    Node.div ~attrs:[ Styles.resize_pane ] [ Bonsai_web_contrib_split_pane.to_vdom pane ]
  in
  view, Bonsai_web_contrib_split_pane.inject_set_size pane
;;

let create_parameters_form (local_ graph) =
  let form =
  ...
  in
  let form =
    Bonsai_web_form.With_automatic_view.Dynamic.with_default
  ...
  let form_vdom = Bonsai_web_auto_generated.view_as_vdom form in
  let error =
    match Bonsai_web_form.With_automatic_view.value form with
    | Ok _ -> Node.none
