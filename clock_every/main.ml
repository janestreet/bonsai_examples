  ...
  let form (local_ graph) =
    Form.Typed.Record.make
      (module struct
        ...
        let time_span_form ~max ~default (local_ graph) =
          let form =
...
let timeline ~now ~(tracks : Bar.t Fdeque.t Track_id.Map.t Bonsai.t) (local_ graph) =
  ...
  let tracks =
    Bonsai.assoc
      (module Track_id)
      tracks
      ~f:(fun track_id deque (local_ _graph) ->
...
let all_clocks ~trigger_on_activate ~wait_time ~delta_time (local_ graph) =
  List.map
