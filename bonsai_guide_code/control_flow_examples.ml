...
let counter ~step (local_ graph) = State_examples.counter ~step graph |> Tuple2.get1
...
let show_control f (local_ graph) =
...
let show_control_2 f (local_ graph) =
  let form =
...
let show_control_var f (local_ graph) =
  let form =
    Form.Elements.Dropdown.list
...
let maybe_show_dynamic_count show (local_ graph) =
  match%sub show with
  | `Count_by_1 -> counter ~step:(return 1) graph
  | `Count_by_2 -> counter ~step:(return 2) graph
  | `No -> Bonsai.return Vdom.Node.none
