...
module My_variant = struct
  ...
  let form_for_variant
    ...
    | C ->
      Bonsai.map2
        ~f:Form.both
        (E.Number.int ~step:1 ~allow_updates_when_focused:`Always () graph)
        (E.Number.float ~step:1. ~allow_updates_when_focused:`Always () graph)
  ;;
end
...
let form_for_field : type a. a Typed_field.t -> local_ Bonsai.graph -> a Form.t Bonsai.t =
  ...
  | Radiobutton_buttons ->
    E.Radio_buttons.list
      ...
      (Bonsai.return [ "first"; "second"; "third" ])
      graph
  | Time_span -> E.Date_time.time_span ~allow_updates_when_focused:`Always () graph
  | Date -> E.Date_time.date ~allow_updates_when_focused:`Always () graph
  | Date_range -> E.Date_time.Range.date ~allow_updates_when_focused:`Always () graph
  ...
  | Sexp_from_string ->
    E.Textbox.sexpable ~allow_updates_when_focused:`Always (module Sexp) graph
