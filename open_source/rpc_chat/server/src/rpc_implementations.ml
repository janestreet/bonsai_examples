...
let message_stream global_state =
  ...
  let f _user_state () = Async_bus.pipe1_exn bus_read |> Deferred.Result.return in
  Rpc.Pipe_rpc.implement Protocol.Message_stream.t f ~leave_open_on_exception:true
;;
